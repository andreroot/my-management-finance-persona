...
set DATA_PERIODO_JOBS = DATE_SUB(CURRENT_DATE("America/Sao_Paulo"), INTERVAL %(dias)s day);
...
--DECLARE pattern_sql STRING DEFAULT r"(?i)(create or replace table|create table if not exists|create temp table|merge into|merge)(?:\s+\x60?)(?:composed-night-232419.)?(?:.+?)(?:\x60?\s|\()";
 ...
 create or replace table composed-night-232419.bee_analytics_transporte.information_executor_jobs
 as
